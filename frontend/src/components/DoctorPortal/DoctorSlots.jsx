...
const DoctorSlots = ({ slots_available }) => {
  return (
    <div className="bg-slate-900 p-6 h-full flex flex-col">
      ...
      {slots_available?.length > 0 ? (
        <div className="overflow-y-auto max-h-[550px] pr-2 custom-scrollbar flex-1">
          {/* Responsive Grid */}
        ...
        /* Empty State */
        <div className="flex flex-col items-center justify-center flex-1 min-h-[200px] bg-slate-800/30 rounded-xl border border-dashed border-slate-700">
          <span className="text-3xl mb-3 opacity-50">📭</span>
          <p className="text-slate-400 text-sm font-medium">No open slots available.</p>
          <p className="text-slate-500 text-xs mt-1">Use the generator to add availability.</p>
        </div>
      )}
    </div>
  );
