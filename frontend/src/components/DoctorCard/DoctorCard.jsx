    ...
    <div key={doc._id} className="bg-slate-800 rounded-xl flex items-center border border-slate-700 overflow-hidden shadow-sm transition-all hover:shadow-md hover:border-slate-600 group">
      ...
      <div className="bg-slate-900/50 p-5 border-t  border-slate-700/50 w-full">
        ...
        {doc.slots_available && doc.slots_available.length > 0 ? (
          <ul className="flex flex-wrap gap-3 max-h-[150px] overflow-y-auto pt-1" aria-label="Available appointment slots">
            {doc.slots_available.map((slot, idx) => (
              <li key={idx}>
                <button
                  onClick={() => handleSlotSelection(doc, slot)}
                  className="flex gap-3 w-[220px]  items-center justify-center px-4 py-2 min-w-[90px] bg-slate-800 border border-slate-600 rounded-lg hover:bg-blue-600 hover:border-blue-500 hover:-translate-y-0.5 transition-all duration-200 active:scale-95 group/btn shadow-sm"
                >
                  <span className="text-sm font-bold text-slate-200 group-hover/btn:text-white">{slot.time}</span>
                  <span className="text-sm text-slate-400 group-hover/btn:text-blue-100">{formatDate(slot.date)}</span>
                </button>
              </li>
            ))}
          </ul>
